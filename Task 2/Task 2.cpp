...
using namespace std;
int main()
{ // Beginning of Task 1: In lines 5 & 6 below, I'm declaring and initialising loop variables
	...
	while (height <= 199) // I'm using a 'while' loop structure to create an event-controlled loop
	{
		...
		if (height < shortest) shortest = height; if (height > tallest) tallest = height; //Created an if loop to filter between tallest and shortest heights
		if (height <= 159) HR1++;//organising height ranges into if statements to filter integer values given by user on line 10.
	...
	if (height >= 199) { // creating an output display for the histogram using for loops
		...
		cout << "  Additional Statistics  " << endl; // Beginning of Task 2
